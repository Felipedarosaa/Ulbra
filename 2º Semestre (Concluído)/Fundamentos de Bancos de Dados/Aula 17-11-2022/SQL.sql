...
DELETE
FROM livros
WHERE id_livro >= 2 AND preco > 50.00 AND lancamento < '2022-11-22' -- Ou current_date
SELECT * FROM livros
...
DELETE
FROM livros
WHERE titulo LIKE 'Banco_ de Dados_ Ditribuído_'
...
DELETE
FROM assuntos
WHERE id_assunto <> 'S' OR id_assunto <> 'B' OR id_assunto <> 'P'
...
SELECT max(preco) AS maximo, min(preco) AS minimo, avg(preco) AS media, id_assunto
FROM livros
...
SELECT titulo, preco, preco * 1.10 AS Opcao_1, preco * 1.15 AS Opcao_2, preco * 1.20 AS Opcao_3
FROM livros
...
SELECT nome, cpf, MAX(preco) AS maximo
FROM autores, livros
